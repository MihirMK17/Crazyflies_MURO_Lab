...
from crazyflie_interfaces.msg import FullState, VelocityWorld, Hover, Position, TrajectoryPolynomialPiece
...
class Crazyflie:
    ...
    def cmdHover(self, vx, vy, yawrate, zDistance):
        '''
    	Send a streaming command of body-frame velocity, yawrate and the zDistance.
    	
    	Useful for controlling the crazyflies that only move in a plane.
    	
    	Args:
    	----
    	     vx (float): Velocity in X-direction. Meters/second
    	     vy (float): Velocity in Y-Direciton. Meters/second
    	     yawrate (float): Angular Velocity about Z-axis. rad/second
    	     zDistance (float): Altitude of the crazyflie
        '''
        self.cmdHovermsg.header.stamp = self.node.get_clock().now().to_msg()
        self.cmdHovermsg.vx = vx
        self.cmdHovermsg.vy = vy
        self.cmdHovermsg.yaw_rate = yawrate
        self.cmdHovermsg.z_distance = zDistance
...
class CrazyflieServer(rclpy.node.Node):
    ...
    def __init__(self):
        ...
        self.cmdVelocityWorldPublisher = self.create_publisher(
            VelocityWorld, 'all/cmd_velocity_world', 1)
        self.cmdVelocityWorldMsg = VelocityWorld()
        # self.cmdVelocityWorldMsg.header.seq = 0
        self.cmdVelocityWorldMsg.header.frame_id = '/world'
        
        self.cmdHoverPublisher = self.create_publisher(
            Hover, 'all/cmd_hover', 1)
        self.cmdHovermsg = Hover()
        self.cmdHovermsg.header.frame_id = '/world'
